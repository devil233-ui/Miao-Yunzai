...
export class mysNews extends plugin {
  ...
  async setActivityPush() {
    ...
    if (this.e.msg.includes("开启")) {
      ...
      model = "关闭"
      msg += model
      cfg[typeName][this.e.self_id] = lodash.difference(cfg[typeName][this.e.self_id], [
        this.e.group_id,
      ])
  ...
  async setPush() {
    ...
    if (!Array.isArray(cfg[type][this.e.self_id])) cfg[type][this.e.self_id] = []
    ...
    if (this.e.msg.includes("开启")) {
      ...
      if (lodash.isEmpty(cfg[type][this.e.self_id])) delete cfg[type][this.e.self_id]
    }
